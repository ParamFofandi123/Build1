...
btn_bg_image1 = pygame.transform.scale(bg_image1, (1600, 1000))
btn_bg_image2 = pygame.transform.scale(bg_image2, (1600, 1000))
btn_bg_image3 = pygame.transform.scale(bg_image3, (1600, 1000))
btn_bg_image4 = pygame.transform.scale(bg_image4, (1600, 1000))
...
def stage_select():
    ...
    global char_num2
    global stage
    stage_pick = char_font.render("Pick the stage for the match", True, 'black')
    Stage_btn1 = Button(120, 70, btn_bg_image1, 0.2)
    Stage_btn2 = Button(120, 350, btn_bg_image2, 0.2)
    Stage_btn3 = Button(550, 70, btn_bg_image3, 0.2)
    Stage_btn4 = Button(550, 350, btn_bg_image4, 0.2)
    run = True
    while run:
        gameWindow.blit(bg_end_scaled, (0, 0))
        gameWindow.blit(stage_pick, (230, 10))
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
                pygame.quit()
        if Stage_btn1.draw(gameWindow):
            stage = 1
            click_fx.play()
            run = False
            gameloop()
        if Stage_btn2.draw(gameWindow):
            stage = 2
            click_fx.play()
            run = False
            gameloop()
        if Stage_btn3.draw(gameWindow):
            stage = 3
            click_fx.play()
            run = False
            gameloop()
        if Stage_btn4.draw(gameWindow):
            stage = 4
            click_fx.play()
            run = False
            gameloop()
...
def gameloop():
    ...
    def draw_bg():
        if stage == 1:
            scaled_bg = pygame.transform.scale(bg_list[0], (screen_width, screen_height))
            gameWindow.blit(scaled_bg, (0, 0))
        if stage == 2:
            scaled_bg = pygame.transform.scale(bg_list[1], (screen_width, screen_height))
            gameWindow.blit(scaled_bg, (0, 0))
        if stage == 3:
            scaled_bg = pygame.transform.scale(bg_list[2], (screen_width, screen_height))
            gameWindow.blit(scaled_bg, (0, 0))
        if stage == 4:
            scaled_bg = pygame.transform.scale(bg_list[3], (screen_width, screen_height))
            gameWindow.blit(scaled_bg, (0, 0))
